...
.favorite img {
    width: 100%; /* Ensure the heart icon fills the container */
    height: auto;
    position: absolute; /* Add position absolute for precise positioning */
    top: 0; /* Align to the top of the container */
    left: 0; /* Align to the left of the container */
}

.favorite:hover::before {
    content: ""; /* Add pseudo-element for background color */
    position: absolute; /* Position it absolutely */
    top: 0; /* Align to the top of the container */
    left: 0; /* Align to the left of the container */
    width: 100%; /* Cover the entire container */
    height: 100%; /* Cover the entire container */
    background-color: red; /* Red color with opacity */
    z-index: -1; /* Set z-index behind the image */
    border-radius: 100%; /* Make the shape circular */
}
</style>
<body>
...
<div class="content">
    ...
    $sql = "SELECT favorite.favorite_id, favorite.note_id, notes.user_id, notes.title, notes.text, notes.updated_at 
    FROM favorite
    INNER JOIN notes ON favorite.note_id = notes.note_id
    WHERE user_id = $user_id
    AND NOT EXISTS (
        SELECT 1
        FROM deletednotes
        WHERE deletednotes.note_id = favorite.note_id
    )"; 
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<div class='favorite' onclick='unfavoriteNote(". $row['note_id'].")'><img src='icons/fav.png'></div>";
    ...
    <script>
        ...
        function unfavoriteNote(noteId) {
            ...
            xhr.open("POST", "unfavorite_note.php", true);
